...
import { BENEFICIARY_ONBOARDING_SLIDES } from '../constants/beneficiaryOnboardingSlides';
import { RootStackParamList } from '../navigation/AppNavigator';
import { RADIUS, T } from '../theme/tokens';

type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'BeneficiaryOnboardingGuide'>;
};

export function BeneficiaryOnboardingGuideScreen({ navigation }: Props) {
  const insets = useSafeAreaInsets();
  const [stepIndex, setStepIndex] = useState(0);
  const slide = BENEFICIARY_ONBOARDING_SLIDES[stepIndex];
  const isLast = stepIndex === BENEFICIARY_ONBOARDING_SLIDES.length - 1;

  const finishGuide = () => {
    navigation.reset({
      index: 0,
      routes: [{ name: 'BeneficiaryMain' }],
    });
  };
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      ...
      <View style={styles.dots}>
        {BENEFICIARY_ONBOARDING_SLIDES.map((item, index) => {
