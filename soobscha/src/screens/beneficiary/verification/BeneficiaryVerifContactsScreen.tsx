...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'BeneficiaryVerifContacts'>;
  route: RouteProp<RootStackParamList, 'BeneficiaryVerifContacts'>;
};
...
export function BeneficiaryVerifContactsScreen({ navigation, route }: Props) {
  const returnToReview = route.params?.returnToReview ?? false;
  ...
  const handleDone = () => {
    const nextErrors = validateBeneficiaryVerifContacts(draft);
    ...
    navigation.navigate('BeneficiaryVerifReview');
  };
  ...
  return (
    <>
      <BeneficiaryVerifStepLayout
        title="Контакты"
        subtitle="Через них с вами свяжутся волонтёры и партнёры"
        step={4}
        total={BENEFICIARY_VERIF_STEPS}
        onBack={() => navigation.goBack()}
        onNext={handleDone}
        nextLabel={returnToReview ? 'Сохранить' : 'Далее'}
      >
        <VolunteerVerifInfoBanner
          tone="accent"
